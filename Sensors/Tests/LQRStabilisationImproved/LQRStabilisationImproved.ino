...
// ============================================================================
// IMPROVED LQR POLE STABILISATION + POSITION CONTROL
// ============================================================================
// Key improvements over original:
// 1. Non-zero K4 (angular velocity damping) - CRITICAL for stability
// 2. Cascaded control: prioritizes stabilization over position tracking
// 3. Position control only activates when pole is stable (small angle)
// 4. Soft position control with reduced gains to avoid destabilization
// 5. Better derivative filtering to reduce noise-induced oscillations
// ============================================================================
...
float Kd_pos = 10.0f;          // Position derivative gain (m/s -> rad lean)
...
void setupMotors() {
  ...
  shield1.setMaxAcceleration(1, 400);
...
void setup() {
  ...
  Serial.println("=============================================");
  Serial.println("IMPROVED LQR Cascaded Controller");
  Serial.println("=============================================");
  ...
  Serial.println("  'r' = recalibrate zero position");
  ...
  if (Serial.available() > 0) {
    ...
    switch (c) {
      case 'r':  // Recalibrate
        ...
        Serial.println(">> Recalibrated. All states reset.");
        break;
...
void loop() {
  ...
  int16_t motorCmd = computeCascadedControl(x_filtered, x_dot_filtered, 
                                             theta_filtered, theta_dot_filtered);
