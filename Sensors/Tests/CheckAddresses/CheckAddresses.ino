...
void assignAddress()
{
  ...
  while (1)
  {
    if (allowAddressAssignment(desiredAddress))
    {
      ...
      // the desired address.
      Wire1.beginTransmission(desiredAddress);
...
void identifyDevices()
{
  ...
  for (uint8_t i = 1; i < 128; i++)
  {
    ...
    // Quick check: is there ANY device at this address?
    Wire1.beginTransmission(i);
    uint8_t error = Wire1.endTransmission();
    if (error != 0) { continue; }  // Skip if no device found

    // Now try Motoron-specific communication
    MotoronI2C test(i);
    test.setBus(&Wire1);  // ADD THIS LINE - tell test object to use Wire1
    test.enableCrc();
